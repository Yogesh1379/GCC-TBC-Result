...
public class letter6 {
    ...
    /**
     * Extract the Nth occurrence of the address that appears between a "To" line
     * and the following "Subject" line. Returns a single-line string (words separated by spaces).
     */
    ...
    private static void compareAddresses(String modelAddr, String studentAddr,
        ...
        if (modelAddr == null) modelAddr = "";
        if (studentAddr == null) studentAddr = "";
        modelAddr = modelAddr.trim().replaceAll("\\s+([,;:!?])","$1").replaceAll("\\.(?!\\s|$)",". ");
        studentAddr = studentAddr.trim().replaceAll("\\s+([,;:!?])","$1").replaceAll("\\.(?!\\s|$)",". ");

        para.createRun().setText("  (Model) " + modelAddr);
        para.createRun().addBreak();
        para.createRun().setText("  (Student) ");

        String[] mW = modelAddr.isEmpty() ? new String[0] : modelAddr.split("\\s+");
        String[] sW = studentAddr.isEmpty() ? new String[0] : studentAddr.split("\\s+");
