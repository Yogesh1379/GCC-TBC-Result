...
public class ManualResultWithGrace implements IAutoConstant {
    @Test
    public void resultTest() throws EncryptedDocumentException, IOException {
        File file = new File("F:\\GCC  TBC December 2025\\Result\\AllEnglish30DataManualResultnew.xlsx");
        ...
        header.createCell(11).setCellValue("obj grace");
        header.createCell(12).setCellValue("spd grace");
        header.createCell(13).setCellValue("ESL grace");

        for (int i = 1; i <= rc; i++) {

            Row row = sheet.getRow(i);
            String seatNumber = df.formatCellValue(row.getCell(7));// old 3
            String obj = df.formatCellValue(row.getCell(31));//11
            Double objmarks = Double.valueOf(obj);
            String speed = df.formatCellValue(row.getCell(22));//16
            Double speedMarks = Double.valueOf(speed);
            String eel = df.formatCellValue(row.getCell(27));//18
            Double EmailExcelLetter = Double.valueOf(eel);
            String per = df.formatCellValue(row.getCell(28));//21
...
//            String grade = df.formatCellValue(row.getCell(23));//23
            String presentAbsent = df.formatCellValue(row.getCell(13));//30
//            String graceex = df.formatCellValue(row.getCell(32));//32
//            Double graceExcel = Double.valueOf(graceex);
            String email = df.formatCellValue(row.getCell(30));//13
            Double email1 = Double.valueOf(email);
            String stmt = df.formatCellValue(row.getCell(23));//15
            Double stmt1 = Double.valueOf(stmt);
            String ltr = df.formatCellValue(row.getCell(21));//14
...
//                            row1.createCell(1).setCellValue(graceExcel);
                            ...
                            row1.createCell(11).setCellValue(objGrace1);
                            row1.createCell(12).setCellValue(speedGrace1);
                            row1.createCell(13).setCellValue(EmailExcelLetterGrace1);
...
//							&&grade.equals("C")
        ...
        FileOutputStream fileOut = new FileOutputStream(new File("F:\\GCC  TBC December 2025\\Result\\Result6.xlsx"));
