...
public class letter5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (File studentFile : docxFiles) {
            ...
            // compare entire document, but record mistakes only after first "Encl" line
            boolean seenEncl = false;
            ...
            for (int i = 0; i < maxLines; i++) {
                ...
                compareWords(mLine, sLine, p, wordMistakes, studentFile.getName(), seenEncl);
                if (!seenEncl && (mLine.contains("Encl") || sLine.contains("Encl"))) {
                    seenEncl = true;
                }
            }
    ...
    private static void compareWords(String model, String student,
        ...
        while (i<mW.length || j<sW.length) {
            if (k<lcs.size() && i==lcs.get(k)[0] && j==lcs.get(k)[1]) {
                ...
                // use new highlight style
                highlight(para.createRun(), "["+mW[i]+"/"+sW[j]+"]");
                if (record) mistakes.add(new String[]{fileName,"Wrong Word",mW[i]+"/"+sW[j]});
                i++; j++;
            }
            else if (k<lcs.size() && i<lcs.get(k)[0]) {
                highlight(para.createRun(), "["+mW[i]+"]");
                if (record) mistakes.add(new String[]{fileName,"Missing Word",mW[i]});
                i++;
            }
            else if (k<lcs.size() && j<lcs.get(k)[1]) {
                highlight(para.createRun(), "["+sW[j]+"]");
                if (record) mistakes.add(new String[]{fileName,"Extra Word",sW[j]});
                j++;
            }
            else if (i<mW.length) {
                highlight(para.createRun(), "["+mW[i]+"]");
    ...
    private static List<String> splitRunsIntoLines(XWPFParagraph p) {
        ...
        for (XWPFRun run : p.getRuns()) {
            ...
            for (int j=0; j<brCount; j++) sb.append("\n");
        }
    ...
    private static List<int[]> computeLCS(String[] a, String[] b) {
        ...
        for (int x=1; x<=m; x++){
            for (int y=1; y<=n; y++){
                if (normalize(a[x-1]).equals(normalize(b[y-1]))) dp[x][y]=dp[x-1][y-1]+1;
        ...
        int x=m, y=n;
        while (x>0 && y>0){
            if (normalize(a[x-1]).equals(normalize(b[y-1]))){
                lcs.add(0,new int[]{x-1,y-1}); x--; y--;
